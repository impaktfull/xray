...
| Any `dart:io` client (`package:http`, Cronet, …) | `xray_inspector` (`enableHttpOverride`) |
...
<Note>Available since 0.0.9</Note>

Pass `enableHttpOverride: true` to intercept **all** `dart:io` `HttpClient` traffic in the process — including `package:http`, Cronet, and any other client that delegates to `dart:io`. No changes to individual clients are needed.
